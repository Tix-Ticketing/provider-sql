...
const (
	// CredentialsSourceSingleStoreConnectionSecret indicates that a provider
	// should acquire credentials from a connection secret written by a managed
	// resource that represents a SingleStore server.
	CredentialsSourceSingleStoreConnectionSecret xpv1.CredentialsSource = "SingleStoreConnectionSecret"
)

// ProviderCredentials required to authenticate.
type ProviderCredentials struct {
	// Source of the provider credentials.
	// +kubebuilder:validation:Enum=SingleStoreConnectionSecret
	Source xpv1.CredentialsSource `json:"source"`

	// A CredentialsSecretRef is a reference to a SingleStore connection secret
