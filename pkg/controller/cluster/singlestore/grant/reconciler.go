...
var (
	// SingleStore SHOW GRANTS output (verified against a live v8 instance) differs
	// from MySQL and carries trailing clauses we must tolerate:
	//   - privileges may be prefixed with TRANSFERABLE (when
	//     privilege_transfer_mode=per_privilege); it is not a privilege itself.
	//   - the *.* USAGE line carries a trailing authentication clause, e.g.
	//     "... IDENTIFIED BY PASSWORD '<hash>' REQUIRE SSL" or
	//     "... IDENTIFIED WITH authentication_jwt REQUIRE SSL".
	//   - db/table-scoped lines may end with " WITH GRANT OPTION".
	// The user@host target is matched as a single non-space token; everything
	// after it is captured as a tail and inspected for WITH GRANT OPTION rather
	// than enumerating every possible suffix order.
	grantRegex = regexp.MustCompile(`^GRANT (?:TRANSFERABLE )?(.+) ON (\S+)\.(\S+) TO \S+(.*)$`)
)
...
func parseGrant(grant, dbname string, table string) (privileges []string) {
	...
	if len(matches) == 5 && matches[2] == dbname && matches[3] == table {
		...
		if strings.Contains(matches[4], "WITH GRANT OPTION") {
